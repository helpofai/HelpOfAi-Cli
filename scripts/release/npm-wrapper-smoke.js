...
const packageDir = path.join(repoRoot, "npm", "helpofai");
...
async function main() {
  const tempRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "helpofai-npm-smoke-"));
  ...
  try {
    ...
    await runCommand("npx", ["--no-install", "helpofai", "doctor", "--help"], {
      cwd: installDir,
      env,
    });
    await runCommand("npx", ["--no-install", "helpofai-tui", "--help"], {
      cwd: installDir,
      env,
    });
