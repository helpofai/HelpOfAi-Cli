#!/usr/bin/env python3
"""
HelpOfAi-native PinchBench runner.

Loads PinchBench tasks, runs them through helpofai exec, and grades results.
No OpenClaw dependency.

Usage:
    python scripts/benchmarks/pinchbench_helpofai.py --help
    python scripts/benchmarks/pinchbench_helpofai.py --suite task_calendar
    python scripts/benchmarks/pinchbench_helpofai.py --suite task_calendar,task_stock
    python scripts/benchmarks/pinchbench_helpofai.py --suite all
"""
# /// script
...
def prepare_workspace(task: dict, run_dir: Path, tasks_dir: Path) -> Path:
    ...
    # Initialize git repo so helpofai works
    subprocess.run(["git", "init"], cwd=workspace, capture_output=True, check=False)
    subprocess.run(
        ["git", "config", "user.email", "bench@helpofai"],
...
def run_helpofai(
    ...
    """Run helpofai exec on a task and return the result."""
    cmd = [
        "helpofai", "exec",
        "--auto",
...
def grade_llm_judge(task: dict, workspace: Path, transcript: list, model: Optional[str] = None) -> dict[str, Any]:
    """Use helpofai as an LLM judge to grade a task."""
...
Be strict but fair. Partial credit is OK."""
    ...
    cmd = ["helpofai", "exec", "--auto", "--workspace", str(workspace)]
    if model:
        cmd.extend(["--model", model])
...
def run_benchmark(
    ...
    cw_version = "unknown"
    try:
        vr = subprocess.run(["helpofai", "--version"], capture_output=True, text=True)
        if vr.returncode == 0:
    ...
    for i, task in enumerate(all_tasks, 1):
        ...
        # Run helpofai
        print(f"  Running helpofai exec (timeout: {timeout}s)...")
        result = run_helpofai(workspace, task["prompt"], timeout, model=model)
    ...
    print(f"\n{'='*60}")
    print(f"PINCHBENCH SCORE SUMMARY (HelpOfAi)")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run PinchBench tasks through HelpOfAi (no OpenClaw)"
    )
    parser.add_argument(
        "--tasks-dir",
        type=Path,
    ...
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=Path("./results/pinchbench-helpofai"),
        help="Results output directory",
    )
    parser.add_argument("--model", default=None, help="Model override for helpofai")
    parser.add_argument(
        "--timeout-multiplier",
        type=float,
        default=1.0,
