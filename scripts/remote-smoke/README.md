...
This concretizes `docs/REMOTE_VM_US.md`: a cheap US VPS running the HelpOfAi
...
- `agent-session.sh` — sourceable helper for interactive/tmux agent sessions
  as the `helpofai` user.  Sources `/etc/helpofai/runtime.env` so the
...
- Secrets travel as a chmod-600 file over scp, land in `/etc/helpofai/*.env`
  (0640 root:helpofai), and the transfer file is shredded. Never in argv,
...
- The Rust binary reads only `DEEPSEEK_RUNTIME_TOKEN`/`--auth-token` and
  `--port`; the `HELPOFAI_RUNTIME_*` names in `/etc/helpofai/runtime.env`
  work because the systemd unit expands them into flags. Don't start
  `helpofai serve` by hand and expect the env file to apply.
- `helpofai-runtime.service` hard-fails activation if
  `/home/helpofai/.helpofai` or `/home/helpofai/.deepseek` don't exist
  (`ReadWritePaths`); `setup-vm.sh` pre-creates them.
- Both binaries are required (`helpofai` delegates to `helpofai-tui`).
...
fine-grained PAT (scoped to helpofai/HelpOfAi-Cli: Contents RW, Issues RW,
...
gh issue list --repo helpofai/HelpOfAi-Cli --milestone v0.8.58 \
...
git -C /opt/whalebro/helpofai fetch origin
git -C /opt/whalebro/helpofai worktree add \
...
# 6. Deliver
gh pr create --repo helpofai/HelpOfAi-Cli --base main \
