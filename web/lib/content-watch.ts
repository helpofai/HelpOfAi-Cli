...
const LINK_TARGETS: { url: string; label: string }[] = [
  { url: "https://github.com/helpofai/HelpOfAi-Cli", label: "Main repo" },
  { url: "https://github.com/helpofai/HelpOfAi-Cli/issues", label: "Issues" },
  { url: "https://github.com/helpofai/HelpOfAi-Cli/pulls", label: "Pull Requests" },
  { url: "https://github.com/helpofai/HelpOfAi-Cli/discussions", label: "Discussions" },
  { url: "https://github.com/helpofai/HelpOfAi-Cli/releases", label: "Releases" },
  { url: "https://github.com/helpofai/HelpOfAi-Cli/blob/main/LICENSE", label: "License file" },
  { url: "https://github.com/helpofai/HelpOfAi-Cli/blob/main/CODE_OF_CONDUCT.md", label: "Code of Conduct" },
  { url: "https://github.com/helpofai/HelpOfAi-Cli/blob/main/SECURITY.md", label: "Security policy" },
  { url: "https://github.com/helpofai/HelpOfAi-Cli/blob/main/CONTRIBUTING.md", label: "Contributing guide" },
  { url: "https://github.com/helpofai/HelpOfAi-Cli/blob/main/.github/PULL_REQUEST_TEMPLATE.md", label: "PR template" },
  { url: "https://github.com/helpofai/homebrew-deepseek-tui", label: "Homebrew tap" },
  { url: "https://github.com/sponsors/helpofai", label: "Support link (GitHub Sponsors)" },
  { url: "https://buymeacoffee.com/helpofai", label: "Support link (BMC)" },
  { url: "https://registry.npmjs.org/helpofai", label: "npm package (registry API)" },
...
const SEMANTIC_DRIFT_PROMPT = `You are reviewing copy on a community website (helpofai.net) for the open-source HelpOfAi project.
...
export async function runSemanticDrift(env: WatchEnv): Promise<{ ok: boolean; drafted: number; reason?: string }> {
  ...
  const ghHeaders: Record<string, string> = {
    ...
    "User-Agent": "helpofai-web-semantic-drift",
  };
  ...
  const [changelog, commits, homepageHtml, docsHtml] = await Promise.all([
    fetch("https://raw.githubusercontent.com/helpofai/HelpOfAi-Cli/main/CHANGELOG.md", { headers: ghHeaders }).then((r) => r.ok ? r.text() : "").catch(() => ""),
    fetch("https://api.github.com/repos/helpofai/HelpOfAi-Cli/commits?per_page=30", { headers: ghHeaders }).then((r) => r.ok ? r.json() as Promise<{ commit: { message: string }; sha: string }[]> : []).catch(() => []),
    fetch("https://helpofai.net/en", { headers: { "User-Agent": "helpofai-watch" } }).then((r) => r.ok ? r.text() : "").catch(() => ""),
    fetch("https://helpofai.net/en/docs", { headers: { "User-Agent": "helpofai-watch" } }).then((r) => r.ok ? r.text() : "").catch(() => ""),
  ]);
...
${docsText}`;
  ...
  for (const d of drifts) {
    ...
    const draft: AgentDraft = {
      id,
      type: "triage",
      targetUrl: `https://helpofai.net/en/${d.page === "homepage" ? "" : d.page}`,
      bodyEn: body,
      bodyZh: body,
      generatedAt: new Date().toISOString(),
      posted: false,
