...
const FALLBACK_DISPATCH_EN: CuratedDispatch = {
  ...
  highlights: [
    ...
    { title: "Open issues", href: "https://github.com/helpofai/HelpOfAi-Cli/issues", tag: "opened", blurb: "Start with anything labelled good first issue." },
...
const FALLBACK_DISPATCH_ZH: CuratedDispatch = {
  ...
  highlights: [
    ...
    { title: "开放议题", href: "https://github.com/helpofai/HelpOfAi-Cli/issues", tag: "opened", blurb: "从 good first issue 标签开始。" },
        ...
        <div className="mx-auto max-w-[1400px] px-4 sm:px-6 pt-10 sm:pt-14 pb-12 grid lg:grid-cols-12 gap-10">
          <div className="lg:col-span-7">
            ...
            <p className="mt-6 text-lg text-ink-soft leading-relaxed max-w-2xl">
              <span className="font-cjk text-indigo font-semibold">HelpOfAi</span>
              {isZh
            ...
            <div className="mt-6 px-4 py-3 hairline-t hairline-b hairline-l hairline-r text-sm leading-relaxed max-w-2xl space-y-1.5">
              ...
              {isZh ? (
                <>
                  <div><span className="text-jade font-mono text-xs mr-2">✓</span>当前推荐：<span className="font-mono">GLM 5.2</span>，<span className="font-mono">high</span> reasoning，在 HelpOfAi 里表现很好。</div>
                  ...
                  <div className="pt-1 text-xs text-ink-mute">没看到你想要的模型或端点？<a href="https://github.com/helpofai/HelpOfAi-Cli/issues/new" className="text-indigo hover:underline">开个 issue</a>。</div>
                </>
              ) : (
                <>
                  <div><span className="text-jade font-mono text-xs mr-2">✓</span>Recommended right now: <span className="font-mono">GLM 5.2</span> on <span className="font-mono">high</span> reasoning — working very well in HelpOfAi.</div>
                  ...
                  <div className="pt-1 text-xs text-ink-mute">Don&apos;t see a model or endpoint you want? <a href="https://github.com/helpofai/HelpOfAi-Cli/issues/new" className="text-indigo hover:underline">Open an issue</a>.</div>
                </>
              )}
            </div>
            ...
            <div className="mt-6 flex items-center gap-4 text-xs font-mono text-ink-mute flex-wrap">
              <span>{isZh ? "由 helpofai 维护" : "Maintained by helpofai"}</span>
              ...
              <pre className="code-block mt-2">
                {isZh ? (
                  <>
                    ...
                    <span className="prompt">$</span> npm install -g helpofai{"\n"}
                    <span className="prompt">$</span> helpofai{"\n"}
                    <br />
        ...
        <div className="grid md:grid-cols-3 gap-0 col-rule hairline-t hairline-b">
          {(isZh
            ? [
                ...
                { n: "02", t: "配密钥", d: "helpofai auth set --provider deepseek（或 openrouter、anthropic、zai 等）。本地 vLLM / Ollama 无需密钥。", cta: "提供商列表 →", href: isZh ? "/zh/docs#providers" : "/docs#providers" },
                { n: "03", t: "运行", d: "在项目目录里敲 helpofai。默认 Plan 只读调查；按 Tab 切到 Agent 执行。", cta: "文档 →", href: isZh ? "/zh/docs" : "/docs" },
              ]
            : [
                ...
                { n: "02", t: "Add a key", d: "helpofai auth set --provider deepseek (or openrouter, anthropic, zai, …). A local vLLM or Ollama box needs no key at all.", cta: "Provider list →", href: "/docs#providers" },
                { n: "03", t: "Run it", d: "Type helpofai in a project. Plan mode is read-only by default; press Tab for Agent mode.", cta: "Docs →", href: "/docs" },
              ]
          ).map((item) => (
        ...
        {isZh && (
          <div className="mt-8 hairline-t pt-6 flex flex-wrap items-baseline gap-x-6 gap-y-3">
            ...
            <p className="w-full text-xs text-ink-mute leading-relaxed max-w-2xl">
              {isZh
                ? "npm 安装时可设 HELPOFAI_USE_CNB_MIRROR=1，wrapper 会改从 CNB 下载二进制。DeepSeek API（api.deepseek.com）国内直连，无需代理。"
                : "Set HELPOFAI_USE_CNB_MIRROR=1 during npm install to fetch binaries from CNB. The DeepSeek API (api.deepseek.com) is reachable from mainland China without a proxy."}
            </p>
          </div>
        )}
      </section>
      ...
      <section id="release-credits" className="mx-auto max-w-[1400px] px-6 py-14">
        ...
        <div className="grid lg:grid-cols-12 gap-10">
          <div className="lg:col-span-5">
            ...
            <Link href="https://github.com/helpofai/HelpOfAi-Cli/blob/main/CHANGELOG.md" className="inline-block mt-4 font-mono text-xs uppercase tracking-wider text-indigo hover:underline">
        ...
        <div className="mx-auto max-w-[1400px] px-6 py-20 grid lg:grid-cols-12 gap-10 items-center">
          <div className="lg:col-span-8">
            ...
            <h2 className="font-display text-paper text-4xl leading-tight">
              {isZh ? "来自世界各地的开发者，把 HelpOfAi 变成了今天的样子。" : "Developers from all over have made HelpOfAi what it is."}
            </h2>
            <p className={`mt-5 text-paper-deep/80 max-w-2xl ${isZh ? "leading-[1.9]" : "leading-relaxed"}`}>
              {isZh
                ? "HelpOfAi 起于一个人的 DeepSeek 业余项目，来自许多国家的贡献者把它塑造成了现在的样子——每一版发布上的贡献者名单就是证据。这里没有 CLA，没有赞助商优先通道，议题公开分类，版本从 main 分支发布。"
                : "HelpOfAi started as one person's DeepSeek side project, and contributors from many countries have shaped it into what it is — the contributor list on every release is the proof. No CLA, no sponsor lockouts, issues triaged in the open, releases cut from main."}
            </p>
            <blockquote className={`mt-6 pl-4 border-l-2 border-indigo text-paper-deep/90 max-w-2xl ${isZh ? "leading-[1.9]" : "leading-relaxed"}`}>
              {isZh
                ? "「所有反馈都是礼物。」——这是我教书时很早就学到的一句话，也是我维护 HelpOfAi 的方式。每一个 issue、PR、bug 报告或想法，都是我理解真实使用场景、做出更好决策的方式。"
                : "“All feedback is a gift.” Something I learned early in teaching, and the way I maintain HelpOfAi. Every issue, PR, bug report, or idea is how I understand real use cases and make better decisions."}
            </blockquote>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              ...
              <Link
                href="https://github.com/helpofai/HelpOfAi-Cli/issues/new"
