...
/// 2. Writes a crash dump to `~/.helpofai/crashes/<timestamp>-<name>.log`.
///
...
/// best-effort crash dump to `~/.helpofai/crashes/`, so diagnostics land in
...
/// Write a panic dump file to `~/.helpofai/crashes/`.
///
...
fn write_panic_dump(
    ...
    let crash_dir = home.join(".helpofai").join("crashes");
    if !crash_dir.exists() {
...
/// panics, a crash dump is written to `~/.helpofai/crashes/` and the panic
