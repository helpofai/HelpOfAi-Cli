...
fn get_xml_tool_call_regex() -> &'static Regex {
    XML_TOOL_CALL_REGEX.get_or_init(|| {
        // Match <helpofai:tool_call>...</helpofai:tool_call> or similar XML patterns
        Regex::new(r"(?s)<(?:helpofai:)?tool_call[^>]*>\s*(.*?)\s*</(?:helpofai:)?tool_call>")
...
pub fn parse_tool_calls(text: &str) -> ParseResult {
    ...
    // Parse XML-style <helpofai:tool_call> or <tool_call> format
...
pub fn has_tool_call_markers(text: &str) -> bool {
    text.contains("[TOOL_CALL]")
        || text.contains("<helpofai:tool_call")
        || text.contains("<tool_call")
