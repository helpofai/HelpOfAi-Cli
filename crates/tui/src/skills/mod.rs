...
pub fn default_skills_dir() -> PathBuf {
    dirs::home_dir().map_or_else(
        || PathBuf::from("/tmp/helpofai/skills"),
        |p| p.join(".helpofai").join("skills"),
    )
}
...
pub enum SkillDiscoveryMode {
    /// Preserve the existing broad compatibility scan across HelpOfAi,
    /// agentskills.io, Claude, OpenCode, Cursor, and legacy DeepSeek roots.
    Compatible,
    /// Scan only HelpOfAi-owned roots. Callers that also pass an explicit
...
impl SkillDiscoveryMode {
    #[must_use]
    pub fn from_helpofai_only(value: bool) -> Self {
        if value {
            Self::HelpOfAiOnly
        } else {
            Self::Compatible
...
/// 6. `<workspace>/.helpofai/skills` — HelpOfAi workspace skills.
/// 7. [`agents_global_skills_dir`] — agentskills.io global.
/// 8. `~/.claude/skills` — Claude-ecosystem global (#902).
/// 9. `~/.helpofai/skills` — HelpOfAi global, primary install target.
...
fn skills_directories_with_home_and_mode(
    ...
    let mut candidates = match mode {
        ...
        SkillDiscoveryMode::HelpOfAiOnly => helpofai_workspace_skills_dir(workspace)
            .into_iter()
            .collect(),
    };
    if let Some(home) = home_dir {
        match mode {
            SkillDiscoveryMode::Compatible => {
                ...
                candidates.push(home.join(".helpofai").join("skills"));
                candidates.push(home.join(".deepseek").join("skills"));
            }
            SkillDiscoveryMode::HelpOfAiOnly => {
                candidates.push(home.join(".helpofai").join("skills"));
            }
        }
    } else {
        candidates.push(PathBuf::from("/tmp/helpofai/skills"));
    }
    existing_skill_dirs(candidates)
}

pub(crate) fn helpofai_workspace_skills_dir(workspace: &Path) -> Option<PathBuf> {
    let skills_dir = workspace.join(".helpofai").join("skills");
...
mod tests {
    ...
    #[test]
    fn helpofai_only_mode_ignores_cross_tool_skill_dirs() {
        let tmpdir = TempDir::new().unwrap();
        let workspace = tmpdir.path().join("workspace");
        let home = tmpdir.path().join("home");
        let configured_dir = home.join(".helpofai").join("skills");
        ...
        let registry = super::discover_for_workspace_and_dir_with_home_and_mode(
            ...
            &configured_dir,
            Some(&home),
            super::SkillDiscoveryMode::HelpOfAiOnly,
        );
        let names: Vec<&str> = registry.list().iter().map(|s| s.name.as_str()).collect();

        assert!(names.contains(&"from-helpofai"));
        assert!(names.contains(&"configured-helpofai"));
        assert!(
            !names.contains(&"from-claude") && !names.contains(&"from-agents"),
            "HelpOfAi-only mode must not import cross-tool skills: {names:?}"
        );
    }

    #[test]
    fn helpofai_only_mode_still_honors_explicit_configured_dir() {
        let tmpdir = TempDir::new().unwrap();
        let workspace = tmpdir.path().join("workspace");
        let home = tmpdir.path().join("home");
    ...
    #[test]
    fn helpofai_only_mode_rejects_workspace_helpofai_symlink_escape() {
        let tmpdir = TempDir::new().unwrap();
        let workspace = tmpdir.path().join("workspace");
        let home = tmpdir.path().join("home");
        let escape_target = tmpdir.path().join("escape-target");
        std::fs::create_dir_all(workspace.join(".helpofai")).unwrap();
        ...
        assert!(
            ...
            "HelpOfAi-only mode must not follow workspace .helpofai/skills outside the workspace"
        );
    }
    ...
    /// `.agents/skills/` and a global skill in `~/.helpofai/skills/`
