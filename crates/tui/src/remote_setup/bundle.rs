//! Bundle rendering for `helpofai remote-setup`.
//!
...
//!
//! Env files lead with `HELPOFAI_*` keys; `DEEPSEEK_*` are documented as legacy
...
/// `helpofai_config::provider` registry (the single source of truth).
#[derive(Debug, Clone)]
...
impl ProviderInfo {
    ...
    pub fn from_slug(slug: &str) -> Option<Self> {
        let kind = helpofai_config::ProviderKind::parse(slug)?;
        let p = helpofai_config::provider::provider_for_kind(kind);
        let key_var = p.env_vars().first().copied().unwrap_or("HELPOFAI_API_KEY");
        Some(Self {
            slug: p.id().to_string(),
...
pub fn render_bundle(inputs: &BundleInputs) -> Vec<RenderedFile> {
    vec![
        ...
        RenderedFile {
            relative_path: "helpofai-runtime.service".to_string(),
...
fn render_runtime_env(i: &BundleInputs) -> String {
    ...
    out.push_str("# HelpOfAi runtime config — generated by `helpofai remote-setup`.\n");
    out.push_str("# Provider configuration lives here; the bridge is pure transport.\n");
    out.push_str("# HELPOFAI_* keys are canonical. DEEPSEEK_* are read as legacy aliases.\n\n");

    out.push_str(&format!("HELPOFAI_PROVIDER={}\n", i.provider.slug));
    out.push_str(&format!(
    ...
    out.push_str(&format!(
        "HELPOFAI_MODEL={}   # provider default is {}\n",
    ...
    out.push_str(&format!("HELPOFAI_RUNTIME_TOKEN={}\n", i.runtime_token));
    out.push_str(&format!("HELPOFAI_RUNTIME_PORT={}\n", i.port));
    out.push_str(&format!("HELPOFAI_RUNTIME_WORKERS={}\n", i.workers));
...
fn render_bridge_env(i: &BundleInputs) -> String {
    let mut out = String::new();
    out.push_str(&format!(
        "# HelpOfAi {} bridge config — generated by `helpofai remote-setup`.\n",
        i.bridge.display
    ));
    ...
    out.push_str(&format!("HELPOFAI_RUNTIME_URL={DEFAULT_RUNTIME_URL}\n"));
    out.push_str("# Must match HELPOFAI_RUNTIME_TOKEN in runtime.env.\n");
    out.push_str(&format!("HELPOFAI_RUNTIME_TOKEN={}\n", i.runtime_token));
    out.push_str(&format!("HELPOFAI_WORKSPACE={}\n", i.workspace));
    out.push_str(&format!("HELPOFAI_MODEL={}\n", i.model));
    out.push_str("HELPOFAI_MODE=agent\n");
    out.push_str("HELPOFAI_ALLOW_SHELL=true\n");
    out.push_str("HELPOFAI_TRUST_MODE=false\n");
    out.push_str("HELPOFAI_AUTO_APPROVE=false\n\n");
    ...
    out.push_str("\n# --- bridge tuning ---\n");
    out.push_str(&format!(
        "{}_THREAD_MAP_PATH=/var/lib/helpofai-{}-bridge/thread-map.json\n",
    ...
    out.push_str("HELPOFAI_TURN_TIMEOUT_MS=900000\n");
    out
}
...
fn render_runtime_unit(i: &BundleInputs) -> String {
    format!(
        "[Unit]\n\
Description=HelpOfAi Runtime API\n\
...
# Legacy /etc/deepseek is loaded first for old installs; /etc/helpofai wins.\n\
EnvironmentFile=-/etc/deepseek/runtime.env\n\
EnvironmentFile=-/etc/helpofai/runtime.env\n\
ExecStart=/bin/sh -lc 'exec /home/helpofai/.cargo/bin/helpofai serve --http --host 127.0.0.1 --port \"${{HELPOFAI_RUNTIME_PORT:-${{DEEPSEEK_RUNTIME_PORT:-{port}}}}}\" --workers \"${{HELPOFAI_RUNTIME_WORKERS:-${{DEEPSEEK_RUNTIME_WORKERS:-{workers}}}}}\" --auth-token \"${{HELPOFAI_RUNTIME_TOKEN:-${{DEEPSEEK_RUNTIME_TOKEN}}}}\"'\n\
Restart=on-failure\n\
RestartSec=5\n\
NoNewPrivileges=true\n\
PrivateTmp=true\n\
ProtectSystem=full\n\
ReadWritePaths=/home/helpofai/.helpofai /home/helpofai/.deepseek {workspace}\n\n\
[Install]\n\
...
fn render_bridge_unit(i: &BundleInputs) -> String {
    format!(
        "[Unit]\n\
Description=HelpOfAi {display} Phone Bridge\n\
Wants=network-online.target helpofai-runtime.service\n\
After=network-online.target helpofai-runtime.service\n\n\
[Service]\n\
Type=simple\n\
User=helpofai\n\
Group=helpofai\n\
...
EnvironmentFile=-/etc/helpofai/{slug}-bridge.env\n\
...
PrivateTmp=true\n\
ProtectSystem=full\n\
ReadWritePaths=/var/lib/helpofai-{slug}-bridge\n\n\
[Install]\n\
WantedBy=multi-user.target\n",
...
fn render_runbook(i: &BundleInputs) -> String {
    ...
    out.push_str(&format!(
        "# HelpOfAi remote-setup runbook — {} + {}\n\n",
        i.cloud.display, i.bridge.display
    ));
    out.push_str("Generated by `helpofai remote-setup` (generate-only). Nothing was run on\n");
    ...
    out.push_str("| `helpofai-runtime.service` | systemd unit for the runtime API. |\n");
    out.push_str(&format!(
    ...
    out.push_str(&format!(
        "- A random `HELPOFAI_RUNTIME_TOKEN` was generated (`{}`). It is identical in\n",
        i.runtime_token
    ));
    ...
    out.push_str("```sh\nsudo install -d -m 750 /etc/helpofai\n");
    out.push_str(&format!(
        "sudo install -m 600 runtime.env /etc/helpofai/runtime.env\n\
sudo install -m 600 {slug}.env /etc/helpofai/{slug}-bridge.env\n\
sudo install -m 644 helpofai-runtime.service /etc/systemd/system/helpofai-runtime.service\n\
sudo install -m 644 {unit} /etc/systemd/system/{unit}\n\
sudo systemctl daemon-reload\n\
sudo systemctl enable --now helpofai-runtime {unit}\n```\n\n",
        slug = i.bridge.slug,
...
mod tests {
    ...
    fn bundle_renders_expected_file_set() {
        ...
        assert!(names.contains(&"helpofai-runtime.service"));
        assert!(names.contains(&"helpofai-feishu-bridge.service"));
    ...
    fn env_files_lead_with_helpofai_keys() {
        ...
        let runtime = &files
            ...
            .unwrap()
            .contents;
        assert!(runtime.contains("HELPOFAI_PROVIDER=deepseek"));
        assert!(runtime.contains("HELPOFAI_RUNTIME_TOKEN="));
        assert!(runtime.contains("HELPOFAI_RUNTIME_PORT="));
        ...
        let bridge = &files
            ...
            .unwrap()
            .contents;
        assert!(bridge.contains("HELPOFAI_RUNTIME_URL="));
    ...
    fn every_cloud_bridge_provider_triple_renders() {
        // Cover the matrix per the RFC §Tests; assert HELPOFAI_* + matching token
            ...
            for bridge in &[FEISHU, TELEGRAM] {
                for provider_slug in &["deepseek", "openai", "moonshot"] {
                    ...
                    let runtime = &files
                        ...
                        .unwrap()
                        .contents;
                    assert!(runtime.contains(&format!("HELPOFAI_PROVIDER={provider_slug}")));
                    let token_line = format!("HELPOFAI_RUNTIME_TOKEN={}", inputs.runtime_token);
    ...
    #[test]
    fn systemd_units_reference_helpofai_paths() {
        let inputs = sample_inputs(&LIGHTHOUSE, &FEISHU, "deepseek");
        let files = render_bundle(&inputs);
        let unit = &files
            .iter()
            .find(|f| f.relative_path == "helpofai-runtime.service")
            .unwrap()
            .contents;
        assert!(unit.contains("/etc/helpofai/runtime.env"));
        assert!(unit.contains("HELPOFAI_RUNTIME_TOKEN"));
        // Legacy path still loaded first.
        assert!(unit.contains("/etc/deepseek/runtime.env"));

        let bridge_unit = &files
            .iter()
            .find(|f| f.relative_path == "helpofai-feishu-bridge.service")
            .unwrap()
            .contents;
        assert!(bridge_unit.contains("/etc/helpofai/feishu-bridge.env"));
    }
}
