...
//!
//! Storage shape (TOML at `~/.helpofai/skills_state.toml`, legacy `~/.deepseek/skills_state.toml`):
//!
//! ```toml
...
fn default_state_path() -> Result<PathBuf> {
    let dir = helpofai_config::ensure_state_dir(".")
        .context("could not resolve or create HelpOfAi state directory")?;
