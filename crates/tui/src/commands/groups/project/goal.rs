...
/// Write a legacy trophy card to `~/.helpofai/trophies/<date>-<time>-<slug>.md`
...
fn write_trophy_card(app: &App, verdict: HuntVerdict) -> Result<std::path::PathBuf, String> {
    ...
    let dir = helpofai_config::resolve_state_dir("trophies")
...
fn write_trophy_card_contents(mut f: impl Write, card: TrophyCard<'_>) -> std::io::Result<()> {
    ...
    writeln!(f)?;
    writeln!(f, "_Generated by HelpOfAi `/goal` - {}_", card.now)?;
    Ok(())
}
