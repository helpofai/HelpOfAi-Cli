...
#[given("an offline HelpOfAi evaluation workspace")]
fn offline_helpofai_evaluation_workspace(world: &mut DirectoryListingWorld) {
...
fn user_asks(world: &mut DirectoryListingWorld, prompt: String) {
    ...
    let output = Command::new(helpofai_tui_binary())
        .args(["eval", "--json", "--shell-command", "echo eval-harness"])
        .arg("--record")
        .arg(record_dir.path())
        .output()
        .expect("run helpofai-tui eval");
