...
export function readRuntimeConfig(): RuntimeConfig {
  const config = vscode.workspace.getConfiguration("helpofai");
  const commandPath = config.get<string>("commandPath", "helpofai").trim() || "helpofai";
...
export async function checkRuntime(config: RuntimeConfig): Promise<RuntimeState> {
  ...
  return {
    kind: "connected",
    baseUrl,
    detail: version ? `Connected to HelpOfAi ${version}.` : "Connected to HelpOfAi runtime.",
    version,
  };
}
...
export function startRuntimeTerminal(config: RuntimeConfig): vscode.Terminal {
  const terminal = vscode.window.createTerminal("HelpOfAi Runtime");
  const args = [
    "serve",
    "--http",
    "--host",
...
export function openHelpOfAiTerminal(config: RuntimeConfig): vscode.Terminal {
  const terminal = vscode.window.createTerminal("HelpOfAi");
