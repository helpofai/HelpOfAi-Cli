# HelpOfAi for VS Code

Official HelpOfAi extension scaffold for local development.

This first slice is intentionally small:

- open HelpOfAi in an integrated terminal
- start `helpofai serve --http` in a visible terminal
...
npm run compile
npm run package
code --install-extension helpofai-vscode-0.8.53.vsix
```

Configure `helpofai.commandPath`, `helpofai.runtimeHost`,
`helpofai.runtimePort`, `helpofai.runtimeToken`, and
`helpofai.agentViewRefreshIntervalSeconds` from VS Code settings.
