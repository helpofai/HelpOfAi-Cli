...
- builds Linux x64 release assets from `v*` tags, creates the CNB release, and
  uploads `helpofai-linux-x64`, `helpofai-tui-linux-x64`, and
...
- `HELPOFAI_REPO_BRANCH`: branch or tag to deploy, for example `main`

Optional:

- `HELPOFAI_REPO_URL`: defaults to the CNB mirror URL
- `LIGHTHOUSE_SSH_PORT`: defaults to `22`

The server side should already have `/opt/whalebro/helpofai`,
`/etc/helpofai/runtime.env`, `/etc/helpofai/feishu-bridge.env`, and the
`helpofai-runtime` / `helpofai-feishu-bridge` systemd services from
`docs/TENCENT_LIGHTHOUSE_HK.md`.

## Safety Notes

- Do not store Feishu App Secret or provider API keys in CNB. They belong in
  `/etc/helpofai/*.env` on Lighthouse.
