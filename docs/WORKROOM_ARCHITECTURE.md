...
Workrooms are HelpOfAi's chat-native abstraction for durable, addressable
...
1. **Creation.** A future workroom is created when a thread is started with a
   ...
   is stable and can be shared as a `helpofai://workroom/...` link.
...
3. **Link resolution.** When a `helpofai://workroom/...` link appears in
...
Persisted workroom state should live alongside existing HelpOfAi state:
...
|---|---|
| `helpofai-protocol` | Types: `Workroom`, `WorkroomId`, `WorkroomThread`, `WorkroomEvent`, `WorkroomLink`, `ExternalThreadRef`, `AgentAttribution` |
| `helpofai-app-server` | Future endpoints: `GET /workrooms`, `GET /workroom/:id/threads`, `GET /workroom/resolve` |
| `helpofai-tui` | Future model-facing link resolution and optional sidebar inbox |
| `helpofai-state` | Future: persistent workroom store (Phase 2) |
