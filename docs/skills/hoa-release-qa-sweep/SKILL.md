---
name: hoa-release-qa-sweep
description: "Use before claiming HelpOfAi release work is done: run the full gate sweep and list the manual QA targets."
---

# HelpOfAi Release QA Sweep

Run this before claiming any HelpOfAi release work is "done." A green automated
...
cargo test -p helpofai-config -p helpofai-protocol -p helpofai-cli \
  -p helpofai-helpflow -p helpofai-state --locked

# 3. TUI test binaries, locked
cargo test -p helpofai-tui --bins --locked

# 4. TUI debug build, locked
cargo build -p helpofai-tui --locked
...
# 7. Binary smoke
./target/release/helpofai --version
```
