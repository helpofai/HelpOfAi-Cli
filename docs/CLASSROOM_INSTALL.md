# HelpOfAi Classroom / Lab Install Checklist

A step-by-step checklist for IT admins deploying HelpOfAi on lab or classroom
...
| 4 | Obtain the installer: download `HelpOfAiSetup.exe` from a v0.8.50+ [release](https://github.com/helpofai/HelpOfAi-Cli/releases/latest) or from your department mirror. | ☐ |
| 5 | Verify SHA-256 hash against `helpofai-artifacts-sha256.txt` before deploying. | ☐ |
...
- Installs to `%LOCALAPPDATA%\Programs\HelpOfAi\bin`
...
1. Double-click `HelpOfAiSetup.exe`.
...
```powershell
# 1. Create directory
$binDir = "$env:LOCALAPPDATA\Programs\HelpOfAi\bin"
New-Item -ItemType Directory -Force -Path $binDir

# 2. Download binaries (adjust URL to your mirror or release tag)
$tag = (Invoke-RestMethod -Uri "https://api.github.com/repos/helpofai/HelpOfAi-Cli/releases/latest").tag_name
Invoke-WebRequest -Uri "https://github.com/helpofai/HelpOfAi-Cli/releases/download/$tag/helpofai-windows-x64.exe"     -OutFile "$binDir\helpofai.exe"
Invoke-WebRequest -Uri "https://github.com/helpofai/HelpOfAi-Cli/releases/download/$tag/helpofai-tui-windows-x64.exe" -OutFile "$binDir\helpofai-tui.exe"
...
|---|---------|-----------------|-------|
| 1 | `helpofai --version` | Prints version string | ☐ |
| 2 | `helpofai doctor` | All checks pass | ☐ |
| 3 | `helpofai-tui --version` | Prints version string | ☐ |

If `helpofai` is not found, the user may need to open a **new** terminal window for PATH changes to take effect.

## Lab validation checklist

Run this once on a clean lab machine, and again on a machine that already has a
previous HelpOfAi install:

| # | Scenario | Expected result | Done? |
|---|----------|-----------------|-------|
| 1 | Install with no existing HelpOfAi PATH entry | Adds exactly `%LOCALAPPDATA%\Programs\HelpOfAi\bin` | ☐ |
| 2 | Install twice | PATH is not duplicated | ☐ |
| 3 | Install with a neighboring PATH entry such as `C:\Tools\HelpOfAi\bin-extra` | Neighboring entry is preserved | ☐ |
| 4 | Upgrade by installing a newer `HelpOfAiSetup.exe` over an older one | Apps & Features version and both `--version` outputs match the new build | ☐ |
...
Or create a `config.toml` in `%APPDATA%\helpofai\`:
...
```powershell
& "$env:LOCALAPPDATA\Programs\HelpOfAi\Uninstall.exe" /S
```
...
| Symptom | Fix |
|---------|-----|
| `helpofai` not found after install | Open a **new** terminal. If still missing, check PATH: `echo $env:Path` |
| `MISSING_COMPANION_BINARY` | Ensure both `helpofai.exe` and `helpofai-tui.exe` are in the same directory |
...
| `helpofai doctor` fails API check | Verify `OPENAI_API_KEY` is set or `config.toml` exists |
...
1. Install HelpOfAi using Option A (silent) or Option C (manual).
2. Do **not** set API keys in the image — these are per-user/per-student.
3. The install directory (`%LOCALAPPDATA%\Programs\HelpOfAi\bin`) is per-user,
...
4. Alternatively, install to a shared location like `C:\Tools\HelpOfAi\bin`
   and add it to the **machine** PATH:
   ```powershell
   [Environment]::SetEnvironmentVariable("Path", "$env:Path;C:\Tools\HelpOfAi\bin", "Machine")
   ```
...
|------|-----------------|
| Binaries | `%LOCALAPPDATA%\Programs\HelpOfAi\bin\` |
| User config | `%APPDATA%\helpofai\config.toml` |
| Uninstaller | `%LOCALAPPDATA%\Programs\HelpOfAi\Uninstall.exe` |
