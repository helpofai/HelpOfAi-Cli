...
HelpOfAi nói chuyện native với Anthropic Claude và OpenAI khi đó là thứ bạn
đang có. Công cụ qua cổng phê duyệt, sandbox cấp hệ điều hành, và rollback
bằng `/restore` cho mọi lượt.

[English README](README.md) · [简体中文 README](README.zh-CN.md) · [日本語 README](README.ja-JP.md) · [helpofai.net](https://helpofai.net/) · [Hướng dẫn cài đặt](docs/INSTALL.md) · [Danh mục provider](docs/PROVIDERS.md) · [Changelog](CHANGELOG.md)
...
![HelpOfAi chạy trong terminal](assets/screenshot.png)
...
cài các lệnh `helpofai`, `hoa` và `helpofai-tui`. Muốn tự build từ source?
...
cargo install --git https://cnb.cool/helpofai.net/helpofai --tag v0.8.73 helpofai-cli --locked --force
cargo install --git https://cnb.cool/helpofai.net/helpofai --tag v0.8.73 helpofai-tui --locked --force
...
trong [GitHub Releases](https://github.com/helpofai/HelpOfAi-Cli/releases).
...
`helpofai auth set --provider anthropic` — hoặc chỉ cần export
`ANTHROPIC_API_KEY` — và adapter Messages native sẽ lo phần còn lại.

Key được lưu trong `~/.helpofai/config.toml`; cấu hình cũ trong
...
- `/skills` nạp các workflow tái sử dụng từ `~/.helpofai/skills/`.
- `/config` chỉnh cài đặt runtime; `/statusline` hiển thị route hiện tại,
  chi phí và trạng thái session.
- `! cargo test -p helpofai-tui` chạy bất kỳ lệnh shell nào qua đường
...
- **Hooks v2** *(0.8.58)*. Hook `tool_call_before` trả về quyết định JSON
  ...
  `.helpofai/hooks.toml` riêng cho từng dự án.
...
- **Chế độ headless.** `helpofai exec` với `--allowed-tools`,
...
context dài hơn, tự chủ nhiều hơn. HelpOfAi bắt đầu bằng việc gán trách
nhiệm.
...
của HelpOfAi** cố định thứ tự quyền lực:
...
2. **Luật của repo phải tường minh.** Thêm `.helpofai/constitution.json` để
...
hay OpenAI, HelpOfAi cũng nói các API đó một cách native.
...
trên [helpofai.net](https://helpofai.net/):

- [User guide](docs/GUIDE.md) — giờ đầu tiên với HelpOfAi.
...
- [Sub-agents](docs/SUBAGENTS.md) — vai trò, vòng đời, hợp đồng output và
  hành vi phục hồi.
- [MCP](docs/MCP.md) — kết nối tool server bên ngoài và chạy HelpOfAi như
  một MCP server.
...
- điều phối workflow branch/leaf với HelpFlow.
...
- **[OpenWarp](https://github.com/zerx-lab/warp)** — Cảm ơn vì đã ưu tiên hỗ
  trợ helpofai và hợp tác để mang lại trải nghiệm agent terminal tốt hơn.
...
[danh sách issue đang mở](https://github.com/helpofai/HelpOfAi-Cli/issues) để tìm
những đóng góp đầu tiên phù hợp.

HelpOfAi nhận được rất nhiều báo cáo và PR chất lượng. Lập trường của
...
Ủng hộ dự án: [Buy me a coffee](https://www.buymeacoffee.com/helpofai).
