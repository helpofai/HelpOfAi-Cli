...
test("openharmony x64 resolves to linux x64 binaries", () => {
  withMockedOs("openharmony", "x64", () => {
    ...
    assert.equal(result.helpofai, "helpofai-linux-x64");
    assert.equal(result.tui, "helpofai-tui-linux-x64");
  });
});
...
test("openharmony arm64 resolves to linux arm64 binaries", () => {
  withMockedOs("openharmony", "arm64", () => {
    const { detectBinaryNames } = require(ARTIFACTS_PATH);
    const result = detectBinaryNames();
    assert.equal(result.helpofai, "helpofai-linux-arm64");
    assert.equal(result.tui, "helpofai-tui-linux-arm64");
  });
});
...
test("known platforms are unaffected by alias map", () => {
  for (const [platform, arch, expectedHelpOfAi] of [
    ["linux", "x64", "helpofai-linux-x64"],
    ["darwin", "arm64", "helpofai-macos-arm64"],
    ["linux", "riscv64", "helpofai-linux-riscv64"],
    ["win32", "x64", "helpofai-windows-x64.exe"],
  ]) {
    withMockedOs(platform, arch, () => {
      const { detectBinaryNames } = require(ARTIFACTS_PATH);
      const result = detectBinaryNames();
      assert.equal(result.helpofai, expectedHelpOfAi);
    });
  }
});
...
test("allAssetNames includes every matrix entry", () => {
  ...
  assert.ok(assetNames.includes("helpofai-windows-x64.exe"));
  assert.ok(assetNames.includes("helpofai-tui-windows-x64.exe"));
