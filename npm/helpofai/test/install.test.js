...
async function makeTempDir(t) {
  const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "helpofai-install-test-"));
...
test("install failure hint explains release base override for blocked GitHub downloads", () => {
  ...
  try {
    const error = Object.assign(
      new Error(
        "fetch https://github.com/helpofai/HelpOfAi-Cli/releases/download/v0.8.19/helpofai-artifacts-sha256.txt failed after 5 attempts:\ngetaddrinfo ENOTFOUND github.com",
      ),
      { code: "ENOTFOUND" },
    );
...
test("glibc preflight message is HelpOfAi-branded and actionable", () => {
  const message = glibcInternal.glibcCompatibilityMessage([2, 39, 0], [2, 35, 0]);

  assert.match(message, /Prebuilt HelpOfAi Linux binaries require GLIBC_2\.39/);
  assert.match(message, /this system has glibc 2\.35/);
  assert.match(message, /cargo install helpofai-cli --locked/);
  assert.match(message, /build Linux GNU assets against an older glibc/);
  assert.match(message, /HELPOFAI_SKIP_GLIBC_CHECK=1/);
});
...
test("ensureBinary adopts a manually placed target binary after checksum validation", async (t) => {
  ...
  const target = path.join(dir, process.platform === "win32" ? "helpofai.exe" : "helpofai");
  const assetName = process.platform === "win32" ? "helpofai-windows-x64.exe" : "helpofai-linux-x64";
  const version = "0.8.25";
  const content = Buffer.from("manual helpofai binary");
  let checksumLoads = 0;
  ...
  const result = await withoutForcedDownload(() =>
    _internal.ensureBinary(target, assetName, version, "helpofai/HelpOfAi-Cli", async () => {
      checksumLoads += 1;
