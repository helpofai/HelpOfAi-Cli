function assertSupportedNode() {
  ...
  if (Number.isNaN(major) || major < 18) {
    process.stderr.write(
      "helpofai: Node.js 18 or newer is required for npm installation. " +
      `Current Node.js version is ${version}. ` +
      "Please upgrade Node.js and rerun `npm install -g helpofai`.\n",
    );
    process.exit(1);
  }
}
...
function binaryPaths() {
  ...
  return {
    helpofai: {
      asset: helpofai,
      target: path.join(releaseDir, process.platform === "win32" ? "helpofai.exe" : "helpofai"),
    },
    tui: {
      asset: tui,
      target: path.join(releaseDir, process.platform === "win32" ? "helpofai-tui.exe" : "helpofai-tui"),
    },
  };
}
...
function logInfo(message) {
  ...
  process.stderr.write(`helpofai: ${message}\n`);
}
...
function createProgressReporter(assetName, totalBytes) {
  ...
  const render = (final) => {
    if (totalBytes && totalBytes > 0) {
      ...
      const line = `helpofai: downloading ${assetName}: ${formatMb(received)} / ${formatMb(totalBytes)} MB (${pct}%)`;
      if (interactive) {
      ...
      const line = `helpofai: downloading ${assetName}: ${formatMb(received)} MB downloaded`;
      if (interactive) {
        process.stderr.write(`${line}\r`);
      } else {
        process.stderr.write(`${line}\n`);
  ...
  return {
    ...
    finish() {
      ...
      process.stderr.write(`helpofai: ${assetName} ... done.\n`);
    },
  };
}
...
async function run(options = {}) {
  ...
  await Promise.all([
    ensureBinary(paths.helpofai.target, paths.helpofai.asset, version, repo, getChecksums, { context }),
...
if (require.main === module) {
  run({ context: "install" }).catch((error) => {
    console.error("helpofai install failed:", error.message);
