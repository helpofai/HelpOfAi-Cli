...
或 OpenAI 的 key 时，HelpOfAi 也以原生协议直连。工具经审批放行、操作系统级沙箱，
每一轮都可用 `/restore` 回滚。

[English README](README.md) · [日本語 README](README.ja-JP.md) · [Tiếng Việt README](README.vi.md) · [helpofai.net](https://helpofai.net/) · [安装指南](docs/INSTALL.md) · [Provider 注册表](docs/PROVIDERS.md) · [更新日志](CHANGELOG.md)
...
![HelpOfAi 在终端中运行](assets/screenshot.png)
...
`helpofai`、`hoa` 和 `helpofai-tui` 三个命令。更想从源码构建？用 cargo
（Rust 1.88+）：
...
[GitHub Releases](https://github.com/helpofai/HelpOfAi-Cli/releases)。校验和、
...
`helpofai auth set --provider anthropic`，或直接导出 `ANTHROPIC_API_KEY`，
原生 Messages 适配器会接管后续。

密钥保存在 `~/.helpofai/config.toml`；旧的 `~/.deepseek/` 配置仍会被读取，
保持兼容。
...
- `/skills` 从 `~/.helpofai/skills/` 加载可复用的工作流。
- `/config` 编辑运行时设置；`/statusline` 显示当前路由、费用和会话状态。
- `! cargo test -p helpofai-tui` 让任意 shell 命令走正常的审批与沙箱路径。
...
- **无头模式。** `helpofai exec` 支持 `--allowed-tools`、
...
多数编程 Agent 从加码开始：更多工具、更长上下文、更多自主性。HelpOfAi
从落实责任开始。
...
**HelpOfAi Constitution** 把权威次序固定下来：

1. **用户意图至上。** 你当前的请求高于过期的仓库指引、记忆、先前的交接和
   人格叠加层。
2. **仓库法律必须显式。** 添加 `.helpofai/constitution.json`，声明项目的
...
README 承载理念和最快路径，细节放在文档和 [helpofai.net](https://helpofai.net/)：

- [用户指南](docs/GUIDE.md) —— 上手 HelpOfAi 的第一个小时。
...
- [MCP](docs/MCP.md) —— 接入外部工具服务器，或让 HelpOfAi 自己作为 MCP 服务器运行。
...
- **[OpenWarp](https://github.com/zerx-lab/warp)** — 感谢 OpenWarp 优先支持 helpofai，并一起打磨更好的终端智能体体验。
...
[开放 issue](https://github.com/helpofai/HelpOfAi-Cli/issues) 中寻找适合入门的任务。

HelpOfAi 收到很多高质量的报告和 PR。维护者的姿态是把这扇门一直敞开，
同时守住发布质量：
...
支持项目：[Buy me a coffee](https://www.buymeacoffee.com/helpofai)。
